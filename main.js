...
const DATA_DIR = path.join(app.getPath('home'), '.altcoinchain');
...
function getGethPath() {
  const possiblePaths = [
    path.join(process.resourcesPath, 'geth'),
    path.join(__dirname, '..', 'geth'),
    path.join(__dirname, '..', '..', 'geth'),
    '/opt/Altcoinchain Wallet/resources/geth',
    path.join(app.getAppPath(), '..', 'geth')
  ];

  for (const p of possiblePaths) {
    if (fs.existsSync(p)) {
      console.log('Found geth at:', p);
      return p;
    }
  }
  console.error('Geth not found in any location');
  return null;
}
...
function initializeGenesis() {
  ...
  if (!fs.existsSync(chainDataPath) && genesisPath) {
    ...
    if (gethPath) {
      try {
        execSync(`"${gethPath}" --datadir "${DATA_DIR}" init "${genesisPath}"`, {
          stdio: 'inherit'
        });
        console.log('Genesis initialized successfully');
      } catch (err) {
        console.error('Failed to initialize genesis:', err.message);
      }
    }
  }
}
...
function startGethNode() {
  if (gethProcess) {
    console.log('Geth already running');
    return;
  }

  const gethPath = getGethPath();
  if (!gethPath) {
    console.error('Cannot start geth: binary not found');
    return;
  }
  ...
  console.log('Starting geth with args:', args.join(' '));
  ...
  gethProcess.on('error', (err) => {
    console.error('Failed to start geth:', err.message);
    gethProcess = null;
  });
  ...
  console.log('Geth started with PID:', gethProcess.pid);
}
